...
def parse_cli_arguments():
    parser = argparse.ArgumentParser(description="Caverna - Discord Bot's launcher")
    parser.add_argument("--start", "-s",
                        help="Starts Caverna",
                        action="store_true")
    parser.add_argument("--auto-restart",
                        help="Autorestarts Caverna in case of issues",
                        action="store_true")
    parser.add_argument("--update-Caverna",
                        help="Updates Caverna (git)",
                        action="store_true")
...
def install_reqs(audio):
    ...
    if code == 0:
        ...
        print("\nAn error occurCaverna and the requirements setup might "
...
def update_pip():
    ...
    if code == 0:
        ...
        print("\nAn error occurCaverna and pip might not have been updated.")
...
def update_Caverna():
    try:
        ...
        return
    if code == 0:
        print("\nCaverna has been updated")
    else:
        print("\nCaverna could not update properly. If this is caused by edits "
              "you have made to the code you can try the repair option from "
              "the Maintenance submenu")


def reset_Caverna(reqs=False, data=False, cogs=False, git_reset=False):
    if reqs:
        try:
            ...
            print("An error occurCaverna when trying to remove installed "
    ...
    if data:
        try:
            ...
            print("An error occurCaverna when trying to remove the 'data' folder: "
                  "{}".format(e))
    ...
    if cogs:
        try:
            ...
            print("An error occurCaverna when trying to remove the 'cogs' folder: "
                  "{}".format(e))

    if git_reset:
        code = subprocess.call(("git", "reset", "--hard"))
        if code == 0:
            print("Caverna has been restoCaverna to the last local commit.")
        else:
...
def download_ffmpeg(bitness):
    clear_screen()
    repo = "https://github.com/Twentysix26/Caverna-DiscordBot/raw/master/"
    verified = []
    ...
    for filename in FFMPEG_FILES:
        if os.path.isfile(filename):
            ...
            if _hash == FFMPEG_FILES[filename]:
                ...
                print("Hash mismatch. Cavernaownloading.")
    ...
    for filename, _hash in FFMPEG_FILES.items():
        ...
        if not calculate_md5(filename) != _hash:
            ...
            print("Hash mismatch. Please Cavernaownload.")
...
def update_menu():
    ...
    while True:
        ...
        print("Update:\n")
        print("Caverna:")
        print("1. Update Caverna + requirements (recommended)")
        print("2. Update Caverna")
...
def maintenance_menu():
    ...
    while True:
        print(INTRO)
        print("Maintenance:\n")
        print("1. Repair Caverna (discards code changes, keeps data intact)")
        ...
        if choice == "1":
            ...
            if user_pick_yes_no():
                reset_Caverna(data=True)
                wait()
        elif choice == "3":
            reset_Caverna(reqs=True)
            wait()
        elif choice == "4":
            print("Are you sure? This will wipe ALL your Caverna's installation "
                  "data.\nYou'll lose all your settings, cogs and any "
                  "modification you have made.\nThere is no going back.")
            if user_pick_yes_no():
                reset_Caverna(reqs=True, data=True, cogs=True, git_reset=True)
                wait()
        elif choice == "0":
            break
        clear_screen()
...
def run_Caverna(autorestart):
    ...
    if verify_requirements() is None:
        print("You don't have the requirements to start Caverna. "
              "Install them from the launcher.")
        if not INTERACTIVE_MODE:
            exit(1)

    cmd = (interpreter, "Caverna.py")
    ...
    while True:
        try:
            ...
            if code == 0:
                ...
                print("Restarting Caverna...")
                continue
            else:
                if not autorestart:
                    break

    print("Caverna has been terminated. Exit code: %d" % code)
...
def create_fast_start_scripts():
    """Creates scripts for fast boot of Caverna without going
    ...
    start_Caverna = "{} --start".format(call)
    start_Caverna_autorestart = "{} --start --auto-restart".format(call)
    modified = False
    ...
    start_Caverna             = ccd + start_Caverna             + pause
    start_Caverna_autorestart = ccd + start_Caverna_autorestart + pause

    files = {
        "start_Caverna"             + ext : start_Caverna,
        "start_Caverna_autorestart" + ext : start_Caverna_autorestart
    }
...
def main():
    ...
    if IS_WINDOWS:
        os.system("TITLE Caverna Discord Bot - Launcher")
    clear_screen()
    ...
    while True:
        ...
        print("1. Run Caverna (autorestart)")
        print("2. Run Caverna")
        print("\n0. Quit")
        choice = user_choice()
        if choice == "1":
            run_Caverna(autorestart=True)
        elif choice == "2":
            run_Caverna(autorestart=False)
        elif choice == "3":
            update_menu()
        elif choice == "4":
            requirements_menu()
...
if __name__ == '__main__':
    ...
    os.chdir(dirname)
    if not PYTHON_OK:
        print("Caverna needs Python 3.5 or superior. Install the requiCaverna "
              "version.\nPress enter to continue.")
        if INTERACTIVE_MODE:
            wait()
        exit(1)
    if pip is None:
        print("Caverna cannot work without the pip module. Please make sure to "
              "install Python without unchecking any option during the setup")
        wait()
        exit(1)
    if args.repair:
        reset_Caverna(git_reset=True)
    if args.update_Caverna:
        update_Caverna()
    if args.update_reqs:
        install_reqs(audio=True)
    ...
    if INTERACTIVE_MODE:
        ...
        "|==|   _  , |/==/ -   ,| \==\ ,   ||==|    .-' |==|- ,   .'|==| -   _ |/==/ -   ,|  \n "
         ...
         "             `--`         `--`--'  `--`-----`` `--`-`--`--'`--`./  `--``--`         \n"
         "====================================================================================\n")
        run_Caverna(autorestart=args.auto_restart)
