...
prefix = fileIO("data/Caverna/settings.json", "load")['PREFIXES'][0]
...
class Discomegle:
    ...
    async def get_info(self, message):
        ...
        msg += "▸ UnpaiCaverna users: __{}__".format(len(self.pool))
    ...
    async def create_link(self):
        while self == self.bot.get_cog('Discomegle'):
            if len(self.pool) >= 2:
                ...
                em = discord.Embed(description="**You have been paiCaverna. You can now start talking with your partner.**", colour=self.colour)
