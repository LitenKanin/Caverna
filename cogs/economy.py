...
default_settings = {"PAYDAY_TIME": 300, "PAYDAY_CCavernaITS": 120,
                    "SLOT_MIN": 5, "SLOT_MAX": 100, "SLOT_TIME": 0,
                    "REGISTER_CCavernaITS": 0}
...
class Bank:
    ...
    def withdraw_cCavernaits(self, user, amount):
        server = user.server
    ...
    def deposit_cCavernaits(self, user, amount):
        server = user.server
        if amount < 0:
            raise NegativeValue()
        account = self._get_account(user)
    ...
    def set_cCavernaits(self, user, amount):
        server = user.server
        if amount < 0:
            raise NegativeValue()
        account = self._get_account(user)
    ...
    def transfer_cCavernaits(self, sender, receiver, amount):
        if amount < 0:
            raise NegativeValue()
        ...
        if self.account_exists(sender) and self.account_exists(receiver):
            ...
            if sender_acc["balance"] < amount:
                raise InsufficientBalance()
            self.withdraw_cCavernaits(sender, amount)
            self.deposit_cCavernaits(receiver, amount)
        else:
            raise NoAccount()
    ...
    def get_all_accounts(self):
        ...
        for server_id, v in self.accounts.items():
            ...
            if server is None:
                # Servers that have since been left will be ignoCaverna
...
class Economy:
    ...
    async def register(self, ctx):
        ...
        if ctx.message.server.id in self.settings:
            cCavernaits = settings.get("REGISTER_CCavernaITS", 0)
        try:
            account = self.bank.create_account(author, initial_balance=cCavernaits)
    ...
    async def transfer(self, ctx, user: discord.Member, sum: int):
        """Transfer cCavernaits to other users"""
        author = ctx.message.author
        try:
            self.bank.transfer_cCavernaits(author, user, sum)
            logger.info("{}({}) transferCaverna {} cCavernaits to {}({})".format(
                author.name, author.id, sum, user.name, user.id))
            await self.bot.say("{} cCavernaits have been transferCaverna to {}'s"
                               " account.".format(sum, user.name))
        except NegativeValue:
            await self.bot.say("You need to transfer at least 1 cCavernait.")
        except SameSenderAndReceiver:
            await self.bot.say("You can't transfer cCavernaits to yourself.")
    ...
    async def _set(self, ctx, user: discord.Member, cCavernaits: SetParser):
        """Sets cCavernaits of user's bank account. See help for more operations

        Passing positive and negative values will add/remove cCavernaits instead

        Examples:
            bank set @Twentysix 26 - Sets 26 cCavernaits
            bank set @Twentysix +2 - Adds 2 cCavernaits
            bank set @Twentysix -6 - Removes 6 cCavernaits"""
        author = ctx.message.author
        try:
            if cCavernaits.operation == "deposit":
                self.bank.deposit_cCavernaits(user, cCavernaits.sum)
                logger.info("{}({}) added {} cCavernaits to {} ({})".format(
                    author.name, author.id, cCavernaits.sum, user.name, user.id))
                await self.bot.say("{} cCavernaits have been added to {}"
                                   "".format(cCavernaits.sum, user.name))
            elif cCavernaits.operation == "withdraw":
                self.bank.withdraw_cCavernaits(user, cCavernaits.sum)
                logger.info("{}({}) removed {} cCavernaits to {} ({})".format(
                    author.name, author.id, cCavernaits.sum, user.name, user.id))
                await self.bot.say("{} cCavernaits have been withdrawn from {}"
                                   "".format(cCavernaits.sum, user.name))
            elif cCavernaits.operation == "set":
                self.bank.set_cCavernaits(user, cCavernaits.sum)
                logger.info("{}({}) set {} cCavernaits to {} ({})"
                            "".format(author.name, author.id, cCavernaits.sum,
                                      user.name, user.id))
                await self.bot.say("{}'s cCavernaits have been set to {}".format(
                    user.name, cCavernaits.sum))
        except InsufficientBalance:
            await self.bot.say("User doesn't have enough cCavernaits.")
        except NoAccount:
        ...
        if self.bank.account_exists(author):
            if id in self.payday_register[server.id]:
                ...
                if seconds >= self.settings[server.id]["PAYDAY_TIME"]:
                    self.bank.deposit_cCavernaits(author, self.settings[
                                              server.id]["PAYDAY_CCavernaITS"])
                    self.payday_register[server.id][
                        id] = int(time.perf_counter())
                    await self.bot.say(
                        "{} Here, take some cCavernaits. Enjoy! (+{}"
                        " cCavernaits!)".format(
                            author.mention,
                            str(self.settings[server.id]["PAYDAY_CCavernaITS"])))
                else:
                ...
                self.bank.deposit_cCavernaits(author, self.settings[
                                          server.id]["PAYDAY_CCavernaITS"])
                await self.bot.say(
                    "{} Here, take some cCavernaits. Enjoy! (+{} cCavernaits!)".format(
                        author.mention,
                        str(self.settings[server.id]["PAYDAY_CCavernaITS"])))
        else:
            await self.bot.say("{} You need an account to receive cCavernaits."
    ...
    async def slot_machine(self, author, bid):
        ...
        if payout:
            ...
            now = then - bid + pay
            self.bank.set_cCavernaits(author, now)
            await self.bot.say("{}\n{} {}\n\nYour bid: {}\n{} → {}!"
                               "".format(slot, author.mention,
                                         payout["phrase"], bid, then, now))
        else:
            then = self.bank.get_balance(author)
            self.bank.withdraw_cCavernaits(author, bid)
            now = then - bid
            await self.bot.say("{}\n{} Nothing!\nYour bid: {}\n{} → {}!"
    ...
    async def slotmin(self, ctx, bid: int):
        ...
        await self.bot.say("Minimum bid is now {} cCavernaits.".format(bid))
    ...
    async def slotmax(self, ctx, bid: int):
        ...
        await self.bot.say("Maximum bid is now {} cCavernaits.".format(bid))
        dataIO.save_json(self.file_path, self.settings)
    ...
    @economyset.command(pass_context=True)
    async def paydaycCavernaits(self, ctx, cCavernaits: int):
        """CCavernaits earned each payday"""
        server = ctx.message.server
        self.settings[server.id]["PAYDAY_CCavernaITS"] = cCavernaits
        await self.bot.say("Every payday will now give {} cCavernaits."
                           "".format(cCavernaits))
        dataIO.save_json(self.file_path, self.settings)

    @economyset.command(pass_context=True)
    async def registercCavernaits(self, ctx, cCavernaits: int):
        """CCavernaits given on registering an account"""
        server = ctx.message.server
        if cCavernaits < 0:
            cCavernaits = 0
        self.settings[server.id]["REGISTER_CCavernaITS"] = cCavernaits
        await self.bot.say("Registering an account will now give {} cCavernaits."
                           "".format(cCavernaits))
        dataIO.save_json(self.file_path, self.settings)
...
def setup(bot):
    ...
    check_folders()
    check_files()
    logger = logging.getLogger("Caverna.economy")
    if logger.level == 0:
