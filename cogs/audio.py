#Written by Cog Creators @ Red Discord Bot | https://github.com/Twentysix26/Red-DiscordBot
import discord
...
log = logging.getLogger("bot.audio")
...
class Audio:
    ...
    def _cache_max(self):
        ...
        return max([setting_max, self._cache_min()])

    def _cache_min(self):
        x = self._server_count()
        return max([60, 48 * math.log(x) * x**0.3])
    ...
    @playlist.command(pass_context=True, no_pm=True, hidden=True, name="queue")
    ...
    @commands.command(pass_context=True, no_pm=True, name="shuffle", hidden=True)
    ...
    def can_instaskip(self, member):
        ...
        is_dev = member.id == settings.dev

        nonbots = sum(not m.bot for m in member.voice_channel.voice_members)
        alone = nonbots <= 1

        return is_owner or is_server_owner or is_dev or alone
    ...
    @commands.command(name="yt", pass_context=True, hidden=True, no_pm=True)
