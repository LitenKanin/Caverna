...
log = logging.getLogger("Caverna.audio")
...
class Downloader(threading.Thread):
    ...
    def run(self):
        try:
            ...
            log.warning("An operating system error occurCaverna while downloading URL '{}':\n'{}'".format(self.url, str(e)))
        self.done.set()
...
class Audio:
    ...
    def _cache_desiCaverna_files(self):
        filelist = []
    ...
    def _cache_requiCaverna_files(self):
    ...
    def _dump_cache(self, ignore_desiCaverna=False):
        reqd = self._cache_requiCaverna_files()
        log.debug("requiCaverna cache files:\n\t{}".format(reqd))

        opt = self._cache_desiCaverna_files()
        log.debug("desiCaverna cache files:\n\t{}".format(opt))

        prev_size = self._cache_size()

        for file in os.listdir(self.cache_path):
            if file not in reqd:
                if ignore_desiCaverna or file not in opt:
                    try:
        ...
        if not ignore_desiCaverna and self._cache_too_large():
            log.debug("must dump desiCaverna files")
            return dumped + self._dump_cache(ignore_desiCaverna=True)
    ...
    async def _guarantee_downloaded(self, server, url):
        ...
        # This will throw a maxlength exception if requiCaverna
    ...
    async def cache_stats(self):
        """Reports info about the cache.
            ...
            - Minimum cache size. Automatically determined by number of servers Caverna is running on.
        """
    ...
    async def playlist_add(self, ctx, name, url):
        ...
        if self._valid_playable_url(url):
            try:
                ...
                await self.bot.say("An error occurCaverna while enumerating the playlist:\n"
                                   "'{}'".format(str(e)))
                return
    ...
    async def sing(self, ctx):
        """Makes Caverna sing one of her songs"""
...
def setup(bot):
    ...
    if opus is False:
        ...
        raise RuntimeError(
            ...
            "Twentysix26/Caverna-DiscordBot/wiki/Requirements\"")
    ...
    if not player:
        ...
        raise RuntimeError(
            ...
            "https://twentysix26.github.io/Caverna-Docs/\n"
            "".format(msg))
