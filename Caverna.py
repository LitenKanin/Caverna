...
def check_folders():
    folders = ("data", "data/Caverna", "cogs", "cogs/utils")
...
def interactive_setup(settings):
    ...
    if first_run:
        ...
        print("\nThe configuration is done. Leave this window always open to"
              ...
              "Please read this guide for a good overview on how Caverna works:\n"
              "https://twentysix26.github.io/Caverna-Docs/Kanin_getting_started/\n"
...
def set_cog(cog, value):  # TODO: move this out of Caverna.py
    data = dataIO.load_json("data/Caverna/cogs.json")
    data[cog] = value
    dataIO.save_json("data/Caverna/cogs.json", data)
...
def load_cogs(bot):
    ...
    try:
        registry = dataIO.load_json("data/Caverna/cogs.json")
    except:
        registry = {}
    ...
    if owner_cog is None:
        print("The owner cog is missing. It contains core functions without "
              "which Caverna cannot function. Reinstall.")
        exit(1)

    if bot.settings._no_cogs:
        bot.logger.debug("Skipping initial cogs loading (--no-cogs)")
        if not os.path.isfile("data/Caverna/cogs.json"):
            dataIO.save_json("data/Caverna/cogs.json", {})
        return
    ...
    dataIO.save_json("data/Caverna/cogs.json", registry)
...
def main(bot):
    ...
    print("Just getting some things ready!...")
